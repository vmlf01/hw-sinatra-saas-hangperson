...
describe HangpersonGame do
  ...
  describe 'guessing', :skip => false do
    context 'correctly' do
      before :each do
        @game = HangpersonGame.new('garply')
        @valid = @game.guess('a')
      end
      it 'changes correct guess list', :skip => false do
        expect(@game.guesses).to eq('a')
        expect(@game.wrong_guesses).to eq('')
      end
      it 'returns true', :skip => false do
    ...
    context 'incorrectly' do
      ...
      it 'changes wrong guess list', :skip => false do
    ...
    context 'same letter repeatedly' do
      ...
      it 'does not change correct guess list', :skip => false do
        @game.guess('a')
        expect(@game.guesses).to eq('a')
      end
      it 'does not change wrong guess list', :skip => false do
        @game.guess('q')
        expect(@game.wrong_guesses).to eq('q')
      end
      it 'returns false', :skip => false do
        expect(@game.guess('a')).to be false
        expect(@game.guess('q')).to be false
      end
      it 'is case insensitive', :skip => false do
        expect(@game.guess('A')).to be false
        expect(@game.guess('Q')).to be false
    ...
    context 'invalid' do
      ...
      it 'throws an error when empty', :skip => false do
        expect { @game.guess('') }.to raise_error(ArgumentError)
      end
      it 'throws an error when not a letter', :skip => false do
        expect { @game.guess('%') }.to raise_error(ArgumentError)
      end
      it 'throws an error when nil', :skip => false do
        expect { @game.guess(nil) }.to raise_error(ArgumentError)
      end
    end
  end

  describe 'displayed word with guesses', :skip => false do
    before :each do
  ...
  describe 'game status' do
    ...
    it 'should be win when all letters guessed', :skip => false do
